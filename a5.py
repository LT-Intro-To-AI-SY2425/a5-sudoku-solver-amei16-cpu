...
def BFS(state: Board) -> Board:
    ...
    while queue:
        ...
        if isinstance(current_state.rows[row][col], list) and len(current_state.rows[row][col]) == 0:
            continue

        for val in current_state.rows[row][col][:]:

            new_state = copy.deepcopy(current_state)
            new_state.update(row, col, val)

            if not new_state.failure_test():
